...
const hourFields = [
    "hourBaseSalary", "hourPositionAllowance", "hourMealAllowance",
    "hourFuelSupport", "hourPhoneSupport", "hourOtherSupport",
    "hourKpiBonusA", "hourKpiBonusB", "hourKpiBonusC", "hourKpiBonusD",
] as const;
...
const EditableRowHour = ({ structure, ownerLevel, jobTitleId, gradeId, onSaved }: Props) => {
    ...
    const handleSave = () => {
        ...
        hourFields.forEach((f) => { payload[f] = parseViVN(values[f]); });
    ...
    return (
        <>
            ...
            <style>{`
                .er-input-cell { position: relative; padding: 8px !important; transition: background 0.15s ease; }
                .er-input-cell.er-focused { background: #fafafa !important; }
                .er-input-wrap { position: relative; }
                .num-display { display: block; text-align: right; padding: 0 4px; font-size: 13px; color: #262626; }
                .er-salary-input { font-weight: 500; border-radius: 6px; min-width: 130px; text-align: right; border: none; background: transparent; transition: all 0.2s; color: #262626; }
                .er-salary-input:hover { background: #fafafa; }
                .er-salary-input:focus { background: white; border: none; box-shadow: 0 0 0 1px rgba(235,47,150,0.3), 0 2px 8px rgba(235,47,150,0.08); }
