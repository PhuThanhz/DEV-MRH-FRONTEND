...
const PendingManagerEvaluationPage = ({ isTab }: IProps) => {
            ...
            render: (_: any, record: any) => (
                <Button
                    ...
                    style={{
                        ...
                        boxShadow: record.status === "PENDING_MANAGER_REVIEW" ? "0 2px 6px rgba(22,119,255,0.2)" : "0 1px 2px rgba(0,0,0,0.05)",
                        fontSize: 12,
                        height: 28,
                    }}
                >
    ...
    const pending = records.filter(r => r.status === "PENDING_MANAGER_REVIEW").length;
    const completed = records.filter(r => r.status === "COMPLETED" || r.status === "PENDING_APPROVAL").length;
    const inProgress = records.filter(r =>
        ["MANAGER_REVIEWING"].includes(r.status)
    ).length;

    return (
        <PageContainer title="Danh sách đánh giá (Quản lý trực tiếp)">
            <style>{`
