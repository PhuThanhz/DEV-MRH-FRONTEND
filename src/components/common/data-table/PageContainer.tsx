...
const PageContainer: React.FC<PageContainerProps> = ({
    ...
    return (
        <div className={fullHeight ? "flex flex-col h-full w-full bg-[#f8f9fa]" : "min-h-screen w-full bg-[#f8f9fa]"}>
            {/* Header */}
            {title && (
                <div className="bg-[#f8f9fa] border-b border-gray-200/60 px-4 py-2.5">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
                        <div className="flex items-center gap-2.5">
                            {onBack ? (
                                ...
                                <div className="h-5 w-[4px] bg-gradient-to-b from-pink-400 to-pink-600 rounded-full shadow-[0_0_8px_rgba(236,72,153,0.3)]" />
                            )}
                            
                            <h1 className="text-lg sm:text-[22px] font-bold tracking-tight text-gray-900 leading-tight">
                                {title}
                            </h1>
                        </div>
            ...
            {/* Content */}
            <div className={contentClassName ?? (fullHeight ? "px-4 py-1.5 flex-1 min-h-0" : "px-4 py-3")}>
                {children}
            </div>
        </div>
    );
